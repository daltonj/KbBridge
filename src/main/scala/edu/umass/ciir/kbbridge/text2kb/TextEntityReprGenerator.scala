...
class TextEntityReprGenerator() {

  def createQEntityRepr(query: EntityMention): EntityRepr = {
    EntityRepr(entityName = query.entityName, queryId = Some(query.mentionId))
  }

  def createQVEntityRepr(query: EntityMention): EntityRepr = {
  ...
  def createQVMEntityRepr(query: EntityMention): EntityRepr = {
    val nameVariants = createNameVariantsContext(query)
  ...
  def createQVMSEntityRepr(query: EntityMention): EntityRepr = {
    val nameVariants = createNameVariantsContext(query)
    val neighborHood = convertNeighborNameToEntityRepr(createNeighborNameContext(query, nameVariants.map(_._1)))
