 ...
 * Time: 1:27 PM
 */
object TacMentionLinkerScript2 extends App {
  ...
  def writeAnnotationScript(docsRequiringAnnotation: Iterable[String]) = {
    val outputFile = new File("./scripts/annotate-singleMentions-2012-qvm_local-2")
    val n = 100000
    var curBatch = 0
    var p = new PrintWriter(outputFile.getAbsolutePath() + curBatch.toString + ".sh", "UTF-8")
    for ((docSet, idx) <- (docsRequiringAnnotation grouped 3).zipWithIndex) {
      ...
      sb append " java -server -Xmx6G -Dfile.encoding=utf-8 -cp /work1/allan/jdalton/factorie-kbbridge-plugin/target/factorie-kbbridge-1.0-SNAPSHOT-jar-with-dependencies2.jar cc.factorie.app.nlp.el.TacLinkingMain2 "
      ...
      sb append " ./tac-singleMention-annotations-2012-qvm_local-2"
      sb append " /work1/allan/jdalton/factorie-kbbridge-plugin/tac-nlp-annotations-2013-samfactoriener3"
      sb append " false /work1/allan/jdalton/factorie-kbbridge-plugin/urban-dictionary-EL2013idx"
